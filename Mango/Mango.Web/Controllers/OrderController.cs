    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult GetAll(string status)
        {
            ...
            if (response != null && response.IsSuccess)
            {
                ...
                {
                    case "approved":
                        list = list.Where(u => u.Status == StaticDetails.Status_Approved); break;
                    case "readyforpickup":
                        list = list.Where(u => u.Status == StaticDetails.Status_ReadyForPickup); break;
                    case "cancelled":
                        list = list.Where(u => u.Status == StaticDetails.Status_Cancelled); break;
                    default: break;
                }
            }
            else
